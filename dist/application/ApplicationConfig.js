...
class ApplicationConfig extends _BaseConfig.default {
  constructor(identity, auth, options) {
    super(identity, auth, options); // Authentication is not supported for quickstart
  ...
  getApiBaseUri() {
    return this.auth && this.auth.useLtpa ? `/api/v0002` : `https://${this.getOrgId()}.${this.options.domain}/api/v0002`;
  }
  ...
  static parseEnvVars() {
    ...
    let authToken = process.env.WIOTP_AUTH_TOKEN || null; // Also support WIOTP_API_KEY / WIOTP_API_TOKEN usage
    ...
    let appId = process.env.WIOTP_IDENTITY_APPID || uuidv4(); // Options
    ...
    let verifyCert = process.env.WIOTP_OPTIONS_HTTP_VERIFY || "true"; // String to int conversions
    ...
    let auth = null; // Quickstart doesn't support auth, so ensure we only add this if it's defined
