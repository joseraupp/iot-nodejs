...
class BaseClient extends _events.default.EventEmitter {
  constructor(config) {
    super();
    ...
    this.reconnectLog = 0;
    this.mqtt = null;
    this.lostConnectionLog = new _tinycache.default();
  }
  ...
  connect() {
    ...
    this.mqtt.on('connect', () => {
      ...
      this.emit('connect'); // less than 3 connect attempts you get put to a connect delay of 1 second
      ...
      let connectionLostCount = this.lostConnectionLog.size; // Default is 1 second reconnect period
    ...
    this.mqtt.on('reconnect', () => {
      this.log.info("[BaseClient:onReconnect] MQTT client is reconnecting."); // this.log.debug("[BaseClient:onReconnect] Resubscribe topics:");
    ...
    this.mqtt.on('offline', () => {
      ...
      this.emit('offline'); // Record the disconnect event for 5 minutes
